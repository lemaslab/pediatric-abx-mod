...
# Date:        Aug 23 2017
# IRB:
# Description: modify dermatitis hospital data for import to RedCap
...
# file parameters
data.file.name="baby.hospital_dermatitis.csv";data.file.name
...
# dates
# as character
baby.data$dermatitis_hosp_admit_date=as.character(baby.data$dermatitis_hosp_admit_date)
...
newdata <- test[order(test$baby_id, test$dermatitis_hosp_admit_date),]
head(newdata)

# create "redcap_repeat_instance" variable
dt <- as.data.table(newdata)                                   # Create data.table
setkeyv(dt, c("baby_id", "dermatitis_hosp_admit_date", "dermatitis_hosp_icd"))  # Create key for data.table
...
head(dt3)
dt3[1:30,]
range(dt3$redcap_repeat_instance) # 1 17
...
data.final.01$redcap_repeat_instrument="babyhospital_dermatitis";head(data.final.01);dim(data.final.01);str(data.final.01)
...
# file parameters
data.file.name.export="baby.hospital_dermatitis_23Aug17.csv";data.file.name.export
head(data.final.01.drop)
