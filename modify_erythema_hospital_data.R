...
# Date:        Aug 23 2017
# IRB:
# Description: modify erythema hospital data for import to RedCap
...
# file parameters
data.file.name="baby.hospital_toxicum.csv";data.file.name
...
# dates
# as character
baby.data$toxicum_hosp_admit_date=as.character(baby.data$toxicum_hosp_admit_date)
...
newdata <- test[order(test$baby_id, test$toxicum_hosp_admit_date),]
head(newdata)

# create "redcap_repeat_instance" variable
dt <- as.data.table(newdata)                                   # Create data.table
setkeyv(dt, c("baby_id", "toxicum_hosp_admit_date", "toxicum_hosp_icd"))  # Create key for data.table
...
data.final.01$redcap_repeat_instrument="babyhospital_erythema";head(data.final.01);dim(data.final.01);str(data.final.01)
...
# file parameters
data.file.name.export="baby.hospital_toxicum_23Aug17.csv";data.file.name.export
head(data.final.01.drop)
