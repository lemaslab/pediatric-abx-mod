...
# Date:        Aug 21 2017
# IRB:
# Description: modify ear infection data for import to RedCap
...
# file parameters
data.file.name="baby.clinic_ear.csv";data.file.name
...
# dates
# as character
baby.data$ear_infect_charge_date=as.character(baby.data$ear_infect_charge_date)
...
newdata <- test[order(test$baby_id, test$ear_infect_charge_date),]
head(newdata)

# create "redcap_repeat_instance" variable
dt <- as.data.table(newdata)                                   # Create data.table
setkeyv(dt, c("baby_id", "ear_infect_charge_date", "ear_infection_icd"))  # Create key for data.table
...
head(dt3)
dt3[1:30,]
range(dt3$redcap_repeat_instance) # 1 21
...
data.final.01$redcap_repeat_instrument="babyclinic_ear";head(data.final.01);dim(data.final.01);str(data.final.01)
...
# file parameters
data.file.name.export="baby.clinic_ear_21Aug17.csv";data.file.name.export
head(data.final.01.drop)
