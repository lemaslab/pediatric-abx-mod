...
# file parameters
data.file.name="baby.hospital_ear.csv";data.file.name
...
# dates
# as character
baby.data$ear_hospital_admit_date=as.character(baby.data$ear_hospital_admit_date)
...
newdata <- test[order(test$baby_id, test$ear_hospital_admit_date),]
head(newdata)

# create "redcap_repeat_instance" variable
dt <- as.data.table(newdata)                                   # Create data.table
setkeyv(dt, c("baby_id", "ear_hospital_admit_date", "ear_hospital_icd"))  # Create key for data.table
...
head(dt3)
dt3[1:30,]
range(dt3$redcap_repeat_instance) # 1 28
...
data.final.01$redcap_repeat_instrument="babyhospital_ear";head(data.final.01);dim(data.final.01);str(data.final.01)
...
# file parameters
data.file.name.export="baby.hospital_ear_23Aug17.csv";data.file.name.export
head(data.final.01.drop)
