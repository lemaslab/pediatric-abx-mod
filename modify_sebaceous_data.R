...
# Date:        Aug 23 2017
# IRB:
# Description: modify sebaceous data for import to RedCap
...
# file parameters
data.file.name="baby.clinic_sebaceous.csv";data.file.name
...
# dates
# as character
baby.data$sebaceous_charge_date=as.character(baby.data$sebaceous_charge_date)
...
newdata <- test[order(test$baby_id, test$sebaceous_charge_date),]
head(newdata)

# create "redcap_repeat_instance" variable
dt <- as.data.table(newdata)                                   # Create data.table
setkeyv(dt, c("baby_id", "sebaceous_charge_date", "sebaceous_icd"))  # Create key for data.table
...
data.final.01$redcap_repeat_instrument="babyclinic_sebaceous";head(data.final.01);dim(data.final.01);str(data.final.01)
...
# file parameters
data.file.name.export="baby.clinic_sebaceous_23Aug17.csv";data.file.name.export
head(data.final.01.drop)
