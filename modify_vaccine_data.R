...
# Date:        Aug 15 2017
# IRB:
# Description: modify vaccine data for import to RedCap
...
# file parameters
data.file.name="baby.vaccines.csv";data.file.name

# load data
baby.vaccine <- read.csv(paste(data.dir,data.file.name,sep=""))
head(baby.vaccine)
str(baby.vaccine)

# format variables
# dates
baby.vaccine$immune_date=as.Date(baby.vaccine$immune_date, "%m/%d/%Y")
head(baby.vaccine)
...
newdata <- test[order(test$baby_id, test$immune_date),]
head(newdata)
...
data.final.01$redcap_event_name <- factor(data.final.01$redcap_event_name, levels = c("visit_1_arm_1","visit_2_arm_1","visit_3_arm_1","visit_4_arm_1","visit_5_arm_1","visit_6_arm_1","visit_7_arm_1","visit_8_arm_1","visit_9_arm_1","visit_10_arm_1",
                                   ...
                                   "visit_31_arm_1","visit_32_arm_1","visit_33_arm_1","visit_34_arm_1"));head(data.final.01)

# add other import variables
data.final.01$redcap_repeat_instrument="babyvaccines";head(data.final.01);dim(data.final.01)
...
# file parameters
data.file.name.export="baby.vaccines_15Aug17.csv";data.file.name.export
head(data.final.01)

# write file
write.table(data.final.01, file =(paste(data.dir,data.file.name.export,sep="")),row.names=F, sep=";")
