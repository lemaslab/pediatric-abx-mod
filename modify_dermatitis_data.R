...
# Date:        Aug 21 2017
# IRB:
# Description: modify dermatitis data for import to RedCap
...
# file parameters
data.file.name="baby.clinic_dermatitis.csv";data.file.name
...
# dates
# as character
baby.data$dermatitis_charge_date=as.character(baby.data$dermatitis_charge_date)
...
newdata <- test[order(test$baby_id, test$dermatitis_charge_date),]
head(newdata)

# create "redcap_repeat_instance" variable
dt <- as.data.table(newdata)                                   # Create data.table
setkeyv(dt, c("baby_id", "dermatitis_charge_date", "dermatitis_icd"))  # Create key for data.table
...
data.final.01$redcap_repeat_instrument="babyclinic_dermatitis";head(data.final.01);dim(data.final.01);str(data.final.01)
...
# file parameters
data.file.name.export="baby.clinic_dermatitis_21Aug17.csv";data.file.name.export
head(data.final.01.drop)
